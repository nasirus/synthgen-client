...
class AsyncSynthgenClient:
    ...
    async def create_task(self, model: str, messages: List[Dict[str, str]]) -> str:
        ...
        response = await self._request("POST", "/tasks", json=task_request.model_dump())
        return response["message_id"]

    async def get_task(self, message_id: str) -> TaskResponse:
        """Get task status and result"""
        response = await self._request("GET", f"/tasks/{message_id}")
        return TaskResponse.model_validate(response)

    async def delete_task(self, message_id: str) -> None:
        """Delete a task"""
        await self._request("DELETE", f"/tasks/{message_id}")
    ...
    async def create_batch(
        ...
        response = await self._request("POST", "/batches", files=files)
        return response
    ...
    async def list_batches(self, page: int = 1, page_size: int = 50) -> BatchList:
        """List all batches"""
        response = await self._request(
            "GET", "/batches", params={"page": page, "page_size": page_size}
        )
    ...
    async def export_batch(
        ...
        async with self._client.stream(
            "GET", f"{self.base_url}/batches/{batch_id}/export", params=params
        ) as response:
            response.raise_for_status()
